...
import { createChapterCourseNote, createExerciseExplanationNote } from '../utils/notesStorage';
import { 
  getSavedReadingPreferences, 
  saveReadingPreferences, 
  getProseFontSizeClass, 
  getLineSpacingClass, 
  getFontFamilyClass, 
  getReadingThemeClass 
...
export const CourseViewer: React.FC<CourseViewerProps> = ({
  ...
  const [readingPrefs, setReadingPrefs] = useState<ReadingPreferences>(() => getSavedReadingPreferences());

  const handleUpdateReadingPrefs = (newPrefs: ReadingPreferences) => {
    setReadingPrefs(newPrefs);
    saveReadingPreferences(newPrefs);
  };

  const handleQuickZoom = (delta: number) => {
    const current = readingPrefs.fontScalePercent || 100;
    const next = Math.min(200, Math.max(80, current + delta));
    const nextPrefs: ReadingPreferences = {
      ...readingPrefs,
      fontScalePercent: next,
      fontSize: next <= 85 ? 'sm' : next <= 110 ? 'base' : next <= 130 ? 'lg' : next <= 160 ? 'xl' : '2xl',
    };
    handleUpdateReadingPrefs(nextPrefs);
  };
  ...
  const handleGenerateChapterNote = () => {
    createChapterCourseNote(course.titre, course.id, currentChapter, course.domaineNom);
    triggerNoteToast(`✅ Fiche de cours « ${currentChapter.titre} » générée et enregistrée dans vos Notes !`);
  };

  const handleSaveExerciseNote = (ex: Exercice) => {
    const chosenIdx = exerciseAnswers[ex.id];
    const bonneReponse = ex.reponsesCorrectes.map(i => ex.reponsesPossibles[i]).join(' OU ');
    ...
    triggerNoteToast(`📌 Explication de la question enregistrée dans vos Notes du cours !`);
  };
  ...
  // Theme and typography helpers
  const themeStyles = getReadingThemeClass(readingPrefs.theme);
  const fontSizeClass = getProseFontSizeClass(readingPrefs);
  const lineSpacingClass = getLineSpacingClass(readingPrefs);
  const fontFamilyClass = getFontFamilyClass(readingPrefs);
    ...
    <div className="space-y-6">
      ...
      {noteToast && (
        <motion.div
          ...
          className="bg-emerald-900 text-white p-3.5 rounded-2xl shadow-lg border border-emerald-700 text-xs sm:text-sm font-bold flex items-center justify-between"
        >
          ...
          <button
            ...
            className="px-3 py-1 bg-amber-400 text-slate-950 hover:bg-amber-300 rounded-lg font-black text-xs transition cursor-pointer"
          >
            Ouvrir Mes Notes
          </button>
        </motion.div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          {/* Quick Generate Note Button */}
          <button
            onClick={handleGenerateChapterNote}
            className="flex items-center gap-1.5 text-xs sm:text-sm font-bold text-amber-950 bg-amber-100 hover:bg-amber-200 border border-amber-300 px-3 py-2 rounded-xl transition cursor-pointer"
            title="Générer une note automatique avec les formules et points clés de ce chapitre"
          >
            <Sparkles className="w-4 h-4 text-amber-600" />
            <span>Créer Fiche de Note</span>
          </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8 space-y-5">
          ...
          <ReadingComfortControls
            ...
            onChangePreferences={handleUpdateReadingPrefs}
          />
          ...
          <motion.div
            ...
            style={{ fontSize: `${readingPrefs.fontScalePercent || 100}%` }}
            className={`rounded-3xl p-6 sm:p-8 border shadow-xs space-y-6 transition-all duration-200 ${themeStyles.container} ${fontFamilyClass} ${lineSpacingClass}`}
          >
            <div className="border-b border-slate-200/60 pb-4">
              <div className="flex flex-wrap items-center justify-between gap-2">
                ...
                <div className="flex items-center gap-2">
                  {/* Quick Zoom Stepper inside chapter header */}
                  <div className="flex items-center bg-black/5 dark:bg-white/10 rounded-lg p-0.5 text-xs font-bold border border-black/10 dark:border-white/10">
                    <button
                      onClick={() => handleQuickZoom(-15)}
                      className="px-2 py-0.5 hover:bg-black/10 dark:hover:bg-white/20 rounded transition cursor-pointer"
                      title="Diminuer la taille du texte"
                    >
                      A-
                    </button>
                    <span className="px-1 text-[10px] font-mono opacity-80">
                      {readingPrefs.fontScalePercent || 100}%
                    </span>
                    <button
                      onClick={() => handleQuickZoom(15)}
                      className="px-2 py-0.5 hover:bg-black/10 dark:hover:bg-white/20 rounded transition cursor-pointer"
            ...
            {/* Quick Assimilation & Notes Card */}
            <div className="bg-gradient-to-br from-amber-50 to-orange-50/60 border border-amber-200/90 rounded-2xl p-4 sm:p-5 space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-xs font-black uppercase tracking-wider text-amber-950 flex items-center gap-1.5">
                  <Lightbulb className="w-4 h-4 text-amber-600" />
                  <span>Fiche d'Assimilation & Notes du Chapitre</span>
                </span>
                <button
                  onClick={handleGenerateChapterNote}
                  className="px-3 py-1.5 text-xs font-bold text-amber-950 bg-amber-200 hover:bg-amber-300 rounded-lg transition flex items-center gap-1 cursor-pointer"
                >
                  <Edit3 className="w-3.5 h-3.5" />
                  <span>📌 Créer la Note de Cours</span>
                </button>
              </div>

              {currentChapter.formuleCle && (
                <div className="bg-white/80 p-3 rounded-xl border border-amber-200 text-xs text-slate-900">
                  <span className="font-bold text-amber-900 block">💡 Formule / Règle d'or :</span>
                  <span className="font-mono font-bold text-amber-950 block mt-0.5">{currentChapter.formuleCle}</span>
                </div>
              )}

              {currentChapter.astuceTerrain && (
                <div className="text-xs text-slate-700 leading-relaxed bg-white/60 p-2.5 rounded-xl">
                  <span className="font-bold text-slate-900">🛠️ Astuce de terrain : </span>
                  <span>{currentChapter.astuceTerrain}</span>
                </div>
              )}

              {currentChapter.pointsCles && currentChapter.pointsCles.length > 0 && (
                <div className="space-y-1">
                  <span className="text-[11px] font-bold text-slate-600 uppercase">Points clés à retenir :</span>
                  <ul className="text-xs text-slate-800 space-y-1 pl-3 list-disc">
                    {currentChapter.pointsCles.map((pt, i) => (
                      <li key={i}>{pt}</li>
                    ))}
                  </ul>
                </div>
            ...
            <div
              className={`prose max-w-none space-y-4 ${themeStyles.prose} ${fontSizeClass} ${lineSpacingClass} ${fontFamilyClass}`}
                    ...
                    return (
                      ...
                      >
                        ...
                        {!isSubmitted ? (
                          ...
                          <div className={`p-3.5 rounded-xl border text-xs leading-relaxed mt-2 space-y-2 ${
                            ...
                            <div className="font-bold flex items-center justify-between gap-2">
                              <span>{isCorrect ? '✅ Excellente réponse !' : '💡 Explication pédagogique (Ajoutée à votre carnet de remédiation) :'}</span>
                              <button
                                onClick={() => handleSaveExerciseNote(ex)}
                                className="px-2.5 py-1 text-[11px] font-bold bg-white text-slate-800 border border-slate-300 hover:bg-slate-100 rounded-md shadow-2xs transition flex items-center gap-1 shrink-0 cursor-pointer"
                                title="Sauvegarder cette explication dans mes Notes"
                              >
                                <BookmarkCheck className="w-3 h-3 text-blue-600" />
                                <span>Ajouter à mes Notes</span>
                              </button>
                            </div>
                            <p>{ex.explication}</p>
                          </div>
